...
export async function reconcilePendingPayments(companyId?: string): Promise<ReconciliationSummary> {
  ...
  let query = supabase.from("appointments").select("id, company_id, expires_at, asaas_checkout_id, asaas_payment_id, asaas_environment").eq("status", APPOINTMENT_STATUS.PENDING_PAYMENT).eq("asaas_environment", runtime.environment).limit(100);
  ...
  for (const appointment of appointments ?? []) {
    ...
    try {
      ...
      const credentials = await getCompanyAsaasCredentials(appointment.company_id, appointment.asaas_environment);
